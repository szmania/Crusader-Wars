...
- [ ] 5. **Generate Garrison Army:**
    - [ ] 5.1. Create a new method (e.g., in a new `GarrisonGenerator.cs` class) to create the garrison army composition.
    - [ ] 5.2. This method will take the garrison size, culture, and heritage as input.
    - [ ] 5.3. It will use the existing unit mapper logic to generate a list of culturally and technologically appropriate `Regiment` objects (e.g., basic levies, archers) until the total soldier count matches the garrison size. This generated army will represent the defender inside the settlement.

### Phase 2: Siege Battle File Generation

- [ ] 1. **Modify `BattleFile.cs`:**
    - [ ] 1.1. Add a condition: if `BattleState.IsSiegeBattle` is true, determine the battle type based on the calculated `HoldingLevel` from `Sieges.cs`.
        - [ ] 1.1.1. If `HoldingLevel > 1`, set the `<type>` tag in `tut_battle.xml` to `settlement_standard`.
        - [ ] 1.1.2. If `HoldingLevel <= 1`, set the `<type>` tag to `settlement_unfortified`.
    - [ ] 1.2. Add the `<subculture>` tag to `tut_battle.xml`.
        - [ ] 1.2.1. A new method will be needed in the unit mapper logic (e.g., `UnitMappers_BETA.cs`) to retrieve the subculture for a given faction. This method will parse the loaded `*_Factions.xml` file.
        - [ ] 1.2.2. In `BattleFile.cs`, get the defender's (garrison's) culture and heritage from `Sieges.cs`.
        - [ ] 1.2.3. Use the existing unit mapper to determine the defender's Attila faction name.
        - [ ] 1.2.4. Call the new method to get the subculture string for that faction.
        - [ ] 1.2.5. Write the returned subculture value into a `<subculture>...</subculture>` tag within the `<battle_description>` section of the XML file.
    - [ ] 1.3. Call a new method in `Sieges.cs` to get the final settlement level and variation (e.g., `GetSettlementBattleMap()`). This method will return the tile path (e.g., `terrain/tiles/battle/settlement_western_cities/western_city_a/medium/`) and the level upgrade tag (e.g., `<tile_upgrade>level1</tile_upgrade>`).
    - [ ] 1.4. Call `Sieges.GetEscalationTileUpgrade()` to get the escalation tag (e.g., `<tile_upgrade>escalation3</tile_upgrade>`) and add it to the XML.
    - [ ] 1.5. Add logic to write wall damage tags to `tut_battle.xml` based on the breach status.
        - [ ] 1.5.1. Create a new method in `Sieges.cs` or `Sieges_DataTypes.cs` (e.g., `GetFortificationDamageTags()`) that takes the breach status string ("Intact", "SmallBreach", "LargeBreach") as input.
        - [ ] 1.5.2. This method will return a formatted string containing `<fortification_wall_damage>`, `<fortification_building_damage>`, and `<fortification_wall_breaches>` tags.
        - [ ] 1.5.3. The values should be mapped as follows:
        ...
        - [ ] 1.5.4. In `BattleFile.cs`, call this new method and append the returned string to the battle file content.
